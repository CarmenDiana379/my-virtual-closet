...
export function WardrobeProvider({ children }) {
  ...
  const removeDuplicateFromSell = async (itemName) => {
    ...
    if (duplicate) {
      await deleteDoc(
        doc(db, "users", user.uid, "sellItems", duplicate.id)
      );

      setSellItems((prev) =>
        prev.filter((item) => item.id !== duplicate.id)
      );
    }
  };
  ...
  const removeDuplicateFromBin = async (itemName) => {
    ...
    if (duplicate) {
      await deleteDoc(
        doc(db, "users", user.uid, "binItems", duplicate.id)
      );
  ...
  const removeDuplicateFromWardrobe = async (itemName) => {
    ...
    if (duplicate) {
      await deleteDoc(
        doc(db, "users", user.uid, "wardrobeItems", duplicate.id)
      );
  ...
  const addItem = async (newItem) => {
    ...
    const docRef = await addDoc(
      getUserCollection("wardrobeItems"),
      itemToSave
    );
  ...
  const addWishlistItem = async (item) => {
    ...
    const docRef = await addDoc(
      getUserCollection("wishlistItems"),
      itemToSave
    );
  ...
  const removeWishlistItem = async (id) => {
    ...
    await deleteDoc(
      doc(db, "users", user.uid, "wishlistItems", id)
    );
  ...
  const addRecycleItem = async (item) => {
    ...
    const docRef = await addDoc(
      getUserCollection("recycleItems"),
      recycleItem
    );

    setRecycleItems((prev) => [
      ...prev,
      { ...recycleItem, id: docRef.id }
    ]);
  ...
  const removeRecycleItem = async (id) => {
    ...
    await deleteDoc(
      doc(db, "users", user.uid, "recycleItems", id)
    );
